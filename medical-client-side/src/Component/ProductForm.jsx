...
const AddProductForm=(e)=>{
    e.preventDefault()
    // axios({url: `${import.meta.env.VITE_BACKEND_URL}/add_product`,
...
return (
    <div className=' absolute top-0 right-0 w-screen  h-screen  bg-white  bg-opacity-10 flex justify-end items-center z-10' >
        ...
        <form onSubmit={AddProductForm} id='AddProduct' className={`productForm ${toggleForm? 'FormTransform' : '' } relative flex flex-col gap-14  rounded-l-lg px-5`} style={{ width: '450px', background: '#1D2932' }}>
            ...
            <div className="relative z-0 w-full  group px-2 ">
                <select name="category" id="category" ref={CategoryRef} onChange={cateoryChange} className="block h-10 py-2.5 px-2 w-full text-sm text-gray-900  border-2 rounded-md border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 focus:border-2 focus:rounded-md peer" style={{ background: '#1D2932' }}>
